...
const calculateTotalBalance = users => {
    const sumOfBalance = users.reduce((totalBalance , user) => totalBalance + user.balance , 0);
    return sumOfBalance;
};
...
const getNamesSortedByFriendsCount = users => {
    const arrayOfNameBySorting = users
    .map(user =>({name:user.name , numOfFriends:user.friends.length}))
    .sort((a , b) => a.numOfFriends - b.numOfFriends)
    .map(user => user.name);
    return arrayOfNameBySorting;
};
...
const getSortedUniqueSkills = users => {
    let arrayOfSkills = [];
    users.forEach(user => {
        arrayOfSkills = arrayOfSkills.concat(user.skills);
    })
    const newArrOfSkills = arrayOfSkills 
    .filter((skill , index , self) => {
        return self.indexOf(skill) === index;
    })
    .sort((a, b) => a.localeCompare(b));
    return newArrOfSkills;
}
